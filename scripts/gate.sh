...
run_phase storm 'storm done:.*coldOpen=([0-9]{1,3}|1000)ms coldOpenOk=true.*stormOk=true' EMBER_STORM=1
...
run_phase storm-faces 'storm done:.*coldOpen=([0-9]{1,3}|1000)ms coldOpenOk=true.*facesSpike=ACTIVE.*stormOk=true' EMBER_STORM=1 EMBER_FACES_FORCE=1
...
if [[ ${EMBER_SKIP_PEOPLETEST:-0} == 1 ]]; then
  ...
  run_phase peopletest 'peopletest done: PASS.*cleanupOk=true' EMBER_PEOPLETEST=1
fi
